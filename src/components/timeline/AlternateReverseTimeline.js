...
export default function AlternateReverseTimeline({ steps }) {
  ...
  useEffect(() => {
    // Ensure that steps is an array before mapping over it
    if (Array.isArray(steps)) {
      const newSegments = steps.map((step) => {
        ...
        if (Array.isArray(step.SubSteps)) {
          ...
          } else if (step.SubSteps.some(substep => substep.NotSure === 0)) {
            color_dot = 'primary'; // Use primary or secondary as an equivalent to your blue
          } else if (step.SubSteps.every(substep => substep.Done === 2)) {
            color_dot = 'success'; // Use secondary or primary as an equivalent to your green
          }
          return { state: color_dot, description: step.Title };
        } else {
      ...
      console.error('The steps prop is not an array.', steps);
    }
  }, [steps]);
            ...
            <TimelineDot
              ...
              color={
                segment.state === 'inherit'
                  ...
                  ? 'success' // 'success' is a valid color 
                  : 'inherit'
              }
            />
...
AlternateReverseTimeline.defaultProps = {
  steps: [],
};
